...
import { SCRYPT_PARAMS, hashPassword, verifyPassword, revokeToken, createJWT, verifyJWT, extractUser } from './auth.js';
import { sendEmail, brandedEmailTemplate, tradeConfirmationHtml, accountUpdateHtml, announcementHtml, onboardingWelcomeHtml, passwordResetEmail, emailVerificationEmail, accessApprovedEmail, accessDeniedEmail, PLATFORM_URL, SUPPORT_EMAIL } from './email.js';
...
class Router {
  ...
  async handle(req, res) {
    ...
    // Content-Length pre-check: reject oversized requests before reading any body data.
    // readBody also guards at 1MB via streaming, but this short-circuits earlier when
    // the client advertises a large Content-Length (saves bandwidth + CPU).
    const contentLength = parseInt(req.headers['content-length'] || '0', 10);
    if (contentLength > 1e6) {
      return json(res, 413, { error: 'Request body too large (max 1MB)' }), true;
    }
    ...
    try {
      ...
      // Centralized body-too-large handler — covers all 49 readBody() call sites.
      if (err instanceof BodyTooLargeError) {
        if (!res.writableEnded) json(res, 413, { error: err.message });
        return true;
      }
      throw err; // re-throw anything else to the top-level server error handler
    }
    return false;
  }
}
...
// Sentinel error thrown by readBody when the request body exceeds 1MB.
// Caught centrally in Router.handle() — returns 413 for ALL endpoints automatically.
class BodyTooLargeError extends Error {
  constructor() { super('Request body too large (max 1MB)'); this.status = 413; }
}
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    const rejectOnce = () => { if (!settled) { settled = true; reject(new BodyTooLargeError()); } };
    req.on('data', chunk => {
      data += chunk;
      if (data.length > 1e6) {
    ...
    req.on('error', rejectOnce);  // fires after req.destroy()
    req.on('close', rejectOnce);  // fallback if 'error' never fires
  });
}
...
// ─── EMAIL SERVICE ───
// sendEmail, brandedEmailTemplate, and all HTML builders live in server/email.js.
// DB-dependent orchestrators (sendAccountUpdateEmail, sendAnnouncementEmail, etc.) remain here.

// Master kill switch for all investor notification emails. When false, suppresses all
// investor notification emails; transactional emails (password reset, verify, access) always send.
const INVESTOR_NOTIFICATIONS_ENABLED = true;


// ─── AUTH: FORGOT PASSWORD// ─── AUTH: FORGOT PASSWORD (sends email with code) ───
